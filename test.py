...
len_items_lv = len(items_lv) + 1


#for filtr in range(len_items_lv):
#    if
    ...
    if sheet_items[row][4].value != None and sheet_items[row][23].value != None:
        if sheet_items[row][4].value != sheet_items[row-1][4].value:
            ...
            if sheet_items[row][34].value != None:
                ...
                reciept['summa'] += reciept['line_items'][0]['price'] * sheet_items[row][27].value
            else:
            ...
            if sheet_items[row][34].value != None:
                ...
                reciepts[u-1]['summa'] += lmtms['price'] * sheet_items[row][27].value
            else:
                lmtms['price'] = sheet_items[row][31].value
                reciepts[u-1]['summa'] += lmtms['price'] * sheet_items[row][27].value
            reciepts[u-1]['line_items'].append(lmtms)
        lmtms={'variant_id': None, 'quantity': None, 'price': None, 'line_taxes': [{'id':km_id}]}
        reciepts[u-1]['summa'] = round(reciepts[u-1]['summa'], 2)


len_reciepts = len(reciepts)
for i in range(len_reciepts):
    len_line_items = len(reciepts[i]['line_items'])
    if reciepts[i]['store_id'] == balti_store_id:
        for index_b in range(6, sheet_balti.max_row + 1):
            if reciepts[i]['order'] == sheet_balti[index_b][3].value:
                if sheet_balti[index_b][6].value != '-':
                    kokku = round(sheet_balti[index_b][15].value - sheet_balti[index_b][11].value - sheet_balti[index_b][6].value, 2)
                else:
                    kokku = round(sheet_balti[index_b][15].value - sheet_balti[index_b][11].value, 2)
    elif reciepts[i]['store_id'] == kristiine_store_id:
        for index_k in range(6, sheet_kristiine.max_row + 1):
            if reciepts[i]['order'] == sheet_kristiine[index_k][3].value:
                if sheet_kristiine[index_k][6].value != '-':
                    kokku = round(sheet_kristiine[index_k][15].value - sheet_kristiine[index_k][11].value - sheet_kristiine[index_k][6].value, 2)
                else:
                    kokku = round(sheet_kristiine[index_k][15].value - sheet_kristiine[index_k][11].value, 2)
    for k in range(len_line_items):
        if reciepts[i]['line_items'][k]['price'] > 0:
            reciepts[i]['line_items'][k]['price'] = (reciepts[i]['line_items'][k]['price'] * reciepts[i]['line_items'][k]['quantity']) * 100 / reciepts[i]['summa'] / 100
            reciepts[i]['line_items'][k]['price']  = round((reciepts[i]['line_items'][k]['price'] / reciepts[i]['line_items'][k]['quantity']) * kokku, 2)
            idx_last_item = k
        for req in range(len_items_lv):
            if reciepts[i]['line_items'][k]['variant_id'] == items_lv[req-1]['variants'][0]['sku']:
                ...
                break
    total_el_no_last_i = 0
    for q in range(idx_last_item):
        total_el_no_last_i += reciepts[i]['line_items'][q]['price']*reciepts[i]['line_items'][q]['quantity']
    reciepts[i]['line_items'][idx_last_item]['price'] = round((kokku - total_el_no_last_i) / reciepts[i]['line_items'][idx_last_item]['quantity'], 2)
...
#reciepts[0]['line_items'][0]['item_name'] = 'qwer'

#data_0 = json.dumps(reciepts[0], indent=4)
#print(data_0)
#response_post = requests.post(url=url, headers=headers, data=data_0)
